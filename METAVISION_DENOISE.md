...
本文说明当前项目中接入的 Metavision SDK CV 去噪算法，以及 `choose_form` 中 `Denoise` 和 `Threshold(us)` 的调参方式。

## 当前项目接入方式

当前项目在事件流进入显示和推理之前执行去噪，但 `.aedat4` 回放保持原始 `dv_processing` 播放路径，不启用 Metavision 去噪：
...
`Threshold(us)` 的单位是微秒。`10000us` 等于 `10ms`。
...
| `threshold` | `Threshold(us)` | 活动时间窗口，单位微秒 |

调参建议：

- 噪声仍然很多：适当减小 `Threshold(us)`。
- 目标事件被误删、运动变得断续：适当增大 `Threshold(us)`。
...
| `width` | 自动使用相机宽度 | 事件流宽度 |
| `height` | 自动使用相机高度 | 事件流高度 |
| `threshold` | `Threshold(us)` | 同坐标重复事件的抑制时间窗口，单位微秒 |

调参建议：

- 拖尾仍明显：增大 `Threshold(us)`。
- 细节或快速运动被删太多：减小 `Threshold(us)`。
...
| `width` | 自动使用相机宽度 | 事件流宽度 |
| `height` | 自动使用相机高度 | 事件流高度 |
| `threshold` | `Threshold(us)` | STC 时间窗口，单位微秒 |
| `cut_trail` | 固定为 `True` | 是否强力抑制拖尾 |

调参建议：

- 噪声或拖尾仍多：可先减小 `Threshold(us)`。
- 目标变稀疏、边缘断裂：增大 `Threshold(us)`。
- 建议从 `5000us` 到 `20000us` 之间试起。
...
| `diff_thresh_us` | `Threshold(us)` | 连续周期允许的最大时间差，单位微秒 |

调参建议：

- 普通市电照明频闪：先使用默认 `50Hz` 到 `70Hz`。
- 如果是显示器或 LED 控制器频闪，需要把 `min_freq/max_freq` 改到实际频率附近。
- `Threshold(us)` 越大，对周期波动越宽容；太大可能误伤正常周期性运动，太小可能漏掉不稳定频闪。

注意：当前 UI 只暴露 `Threshold(us)`，`filter_length/min_freq/max_freq` 写在代码中。如需调这些参数，需要扩展 `choose_form` 控件或修改 `backend/Camera.py`。
...
| 现象 | 可能原因 | 处理方式 |
| --- | --- | --- |
| 日志显示 `[NoiseFilter] Disabled` | 当前选择是 `None` | 在 `Denoise` 下拉框选择具体算法 |
| 日志显示 `Enabled` 但画面变化不明显 | 阈值过宽，或者场景本身噪声少 | 改小或改大阈值，对比事件率和画面 |
| 画面变得断续 | 滤波过强 | 增大 `Threshold(us)` 或换成 `Activity` |
