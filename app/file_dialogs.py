from PyQt6.QtWidgets import QFileDialog

from .paths import default_checkpoint_dir, default_onnx_model_dir, default_record_dir


def choose_input_file(parent, initial_dir=None):
    file_path, _ = QFileDialog.getOpenFileName(
        parent,
        "选择事件文件",
        initial_dir or default_record_dir(),
        (
            "事件文件 (*.raw *.hdf5 *.h5 *.aedat4);;"
            "RAW 事件文件 (*.raw);;"
            "H5/HDF5 事件文件 (*.hdf5 *.h5);;"
            "AEDAT4 事件文件 (*.aedat4);;"
            "所有文件 (*)"
        ),
    )
    return file_path


def choose_weights_file(parent, runtime_kind="wsl", initial_dir=None):
    if runtime_kind == "windows":
        title = "选择 Windows ONNX 模型"
        default_dir = default_onnx_model_dir()
        file_filter = (
            "正式 ONNX 模型 (*_native_fps.onnx *_native.onnx);;"
            "全部 ONNX 模型 (*.onnx);;"
            "所有文件 (*)"
        )
    else:
        title = "选择 PyTorch 权重"
        default_dir = default_checkpoint_dir()
        file_filter = "PyTorch 权重 (*.pth);;所有文件 (*)"

    file_path, _ = QFileDialog.getOpenFileName(
        parent,
        title,
        initial_dir or default_dir,
        file_filter,
    )
    return file_path
